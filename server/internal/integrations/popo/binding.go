...
var (
	ErrBindingTokenInvalid       = errors.New("popo: binding token invalid or expired")
	ErrBindingAlreadyAssigned    = errors.New("popo: user id is already bound to a different user")
	ErrBindingNotWorkspaceMember = errors.New("popo: redeemer is not a workspace member")
)
...
func (s *BindingTokenService) Mint(ctx context.Context, workspaceID, installationID pgtype.UUID, popoUserID string) (BindingToken, error) {
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, multicaUserID pgtype.UUID) (RedeemedBindingToken, error) {
	if s.tx == nil {
		return RedeemedBindingToken{}, errors.New("popo: BindingTokenService missing TxStarter")
	}
	...
	if row.ChannelType != string(TypePopo) {
		return RedeemedBindingToken{}, ErrBindingTokenInvalid
	}
