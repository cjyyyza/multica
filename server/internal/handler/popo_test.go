...
func TestPopoMutationHandlersRejectUnconfiguredDeployment(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			method: http.MethodPost,
			path:   "/api/workspaces/x/popo/registrations?agent_id=y",
			body:   `{}`,
			status: http.StatusForbidden,
			run:    (*Handler).CreatePopoRegistration,
		},
		{
			name:   "get registration",
			method: http.MethodGet,
			path:   "/api/workspaces/x/popo/registrations/y",
			status: http.StatusForbidden,
			run:    (*Handler).GetPopoRegistration,
		},
		{
			...
			path:   "/api/workspaces/x/popo/registrations/y",
			status: http.StatusForbidden,
			run:    (*Handler).CancelPopoRegistration,
		},
		{
			name:   "registration progress",
			method: http.MethodPost,
			path:   "/api/popo/bridge/registrations/y/progress",
			body:   `{"qr_url":"https://example.test/qr"}`,
			status: http.StatusServiceUnavailable,
			run:    (*Handler).ProgressPopoRegistration,
		},
		{
			name:   "revoke install",
			method: http.MethodDelete,
