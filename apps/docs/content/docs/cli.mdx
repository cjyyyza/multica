...
| `yixiezuo` | `preview` / `import` / `bridge` | Read one issue, import it explicitly, or run the local bridge for user-requested previews, direct URL imports, and result publication. POPO `/yixiezuo import <issue-url>` reports success or failure after the bridge finishes. | `<issue-url>`, `import --confirm --project-id`, `bridge --once` |
