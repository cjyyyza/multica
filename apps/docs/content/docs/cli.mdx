...
| `yixiezuo` | `status` / `sync` / `pull` / `push` | Sync 易协作 with the Multica kanban from this Windows computer via local `popo-cli pmmcp`. The server never opens 易协作 or popo-cli. | `sync --watch`, `--interval` |
