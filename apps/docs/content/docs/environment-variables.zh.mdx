...
| POPO | `MULTICA_POPO_SECRET_KEY` | base64 编码的 32 字节密钥，加密可选的回环 webhook token；服务端不会调用 dj01bot |
...
配置步骤见 [GitHub 集成](/github-integration#自托管配置)、[飞书 Bot](/lark-bot-integration#自托管配置)、[Slack Bot](/slack-bot-integration#自托管配置)、[Telegram Bot](/telegram-bot-integration#自托管配置)和 [POPO Bot](/popo-bot-integration#自托管)。
